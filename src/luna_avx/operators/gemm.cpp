...
void inline sgemm_4x8(const float *__restrict__ a, const float alpha,
  ...
  __m256 b1 = _mm256_load_ps(&b[K * 1]);
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    __m256 c0 = _mm256_load_ps(&c[(M * i) + 0]);
    __m256 c1 = _mm256_load_ps(&c[(M * i) + 1]);
    __m256 c2 = _mm256_load_ps(&c[(M * i) + 2]);
    __m256 c3 = _mm256_load_ps(&c[(M * i) + 3]);
